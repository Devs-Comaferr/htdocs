...
csrfValidateRequest('clientes.registrar_telefono');

function escape_string_telefono(string $str): string
{
...
$observaciones = isset($_POST['observaciones']) ? escape_string_telefono((string)$_POST['observaciones']) : null;
...
try {
    ...
    asegurarRelacionVisitaPedido($conn, $cod_venta, 'Telefono', $opciones);

    odbc_commit($conn);
    header('Location: visita_pedido.php?msg=tel_ok');
    exit();
} catch (Exception $e) {
    odbc_rollback($conn);
    appLogTechnicalError('registrar_telefono', $e->getMessage());
    header('Location: visita_pedido.php?msg=error');
    exit();
} finally {
}
