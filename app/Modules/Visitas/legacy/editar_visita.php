...
function editarVisitaPrepareExecute($conn, string $sql, array $params = [])
{
...
if ($id_visita <= 0) {
    die('ID de visita invÃƒÆ’Ã‚Â¡lido.');
}
...
$result_assignment = editarVisitaPrepareExecute($conn, $sql_assignment, $assignmentParams);
$assignment = odbc_fetch_array($result_assignment);

// Horarios de disponibilidad y tiempo promedio (tiempo_promedio_visita se asume en horas)
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $estado_visita = normalizarEstadoVisita(trim($_POST['estado_visita']));
    ...
    if (empty($fecha_visita) || empty($hora_inicio_visita) || empty($hora_fin_visita)) {
        ...
        if (empty($error)) {
            ...
            if ($estadoLower == 'descartada') {
                ...
                $result_order = editarVisitaPrepareExecute($conn, $sql_order, [$id_visita]);
            ...
            if (!$skipOverlap) {
                ...
                $result_overlap = editarVisitaPrepareExecute($conn, $sql_overlap, [$id_visita, intval($_SESSION['codigo']), $fecha_visita]);
                $overlap = false;
                $overlapDetails = "";
                if ($result_overlap) {
                    while ($row = odbc_fetch_array($result_overlap)) {
                        ...
                        if ($new_start < $existing_end && $new_end > $existing_start) {
                            $overlap = true;
                            // Recuperar el nombre comercial del cliente de la visita solapada
                            $sql_cliente_overlap = "SELECT nombre_comercial FROM [integral].[dbo].[clientes] WHERE cod_cliente = ?";
                            $result_cliente_overlap = editarVisitaPrepareExecute($conn, $sql_cliente_overlap, [$row['cod_cliente']]);
                            $overlapCliente = "";
                            ...
                            if (!empty($row['cod_seccion'])) {
                                ...
                                $result_overlap_seccion = editarVisitaPrepareExecute($conn, $sql_overlap_seccion, [$row['cod_cliente'], $overlap_seccion]);
    ...
    if (empty($error)) {
        $sql_update = "
            ...
            WHERE id_visita = ?
        ";
        if (!editarVisitaPrepareExecute($conn, $sql_update, [$fecha_visita, $hora_inicio_visita, $hora_fin_visita, $observaciones, $estado_visita, $id_visita])) {
...
<head>
    ...
    $(document).ready(function(){
        $("#hora_inicio_visita").change(function(){
            ...
            var promedio = <?php echo $tiempo_promedio_minutes; ?>; // en minutos
            if(startTime) {
...
<div class="container">
    ...
    <form action="<?= BASE_URL ?>/visitas.php?action=editar&amp;id_visita=<?php echo $id_visita; ?><?php echo isset($_GET['origen']) ? '&amp;origen=' . $_GET['origen'] : ''; ?>" method="POST">
        ...
        <?php 
            // Si venimos de pedidos_visitas, al cancelar se carga el calendario en vista diaria
            if(isset($_GET['origen']) && $_GET['origen'] === 'pedidos_visitas'){
        ...
        <!-- Botn rojo para eliminar visita -->
